...
# Design multilayer perceptron NN 
model <- keras_model_sequential()
model %>%
  layer_dense(units = 200, activation = 'relu', input_shape = ncol(train)) %>%
...
# During the training 40% of neurons are dropped to zero. So they are not used.In other words, dropout layer is used to avoid overfitting. 
...
plot(history)
end.time_ann <- Sys.time()

end.time_ann -start.time_ann
...
table(Predicted = as.matrix(pred) , Actual = as.matrix(testtarget))
